...
public class ListTest1 {
	...
	public static void main(String[] args) {
		...
		//제네릭 클래스는 <> 타입을 클래스로 해야함 => 일반변수 x
		//ArrayList는 <>를 반드시 적어줘야함 
		ArrayList<String> list = new ArrayList<String>();//갯수제한이 없음
		//add() : 리스트에 추가
		for(int i=0;i<10;i++) {
			list.add("학원 출근");
		}
		list.add("수업");//리스트가 꽉 차면 자동으로 크기를 늘림
		//get(번지) : 번지에 있는 값을 가져옴(추가)
		System.out.println(list.get(10));
		System.out.println("리스트 크기 : " + list.size());
		list.add(10,"아침식사");
		System.out.println(list.get(10));
		System.out.println("리스트 크기 : " + list.size());
		//set(번지, 값) : 번지에 값을 설정(덮어씀)
		list.set(10, "간식");
		System.out.println(list.get(10));
		System.out.println("리스트 크기 : " + list.size());
		//indexOf(값) : 값이 리스트에 있는지 없는지 확인하여 있으면 번지,없으면 -1을 반환
