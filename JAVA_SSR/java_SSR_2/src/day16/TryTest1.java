...
public class TryTest1 {
	...
	public static void main(String[] args) {
		int num1 = 1, num2 = 0;
		// 예외 발생 시 catch에서 해당 예외 클래스나 조상 클래스가 있으면 예외를 처리한다.
		// 나누기는 ArithmeticException이 발생할 수 있는 코드이다.
		// ArithmeticException > RuntimeException > Exception
		
		try {
			System.out.println(num1/num2);
		}//Exception은 ArithmeticException의 조상이므로 예외처리가 가능하다.
		catch(Exception e){
		...
		try {
			System.out.println(num1/num2);
		}//RuntimeException은 ArithmeticException의 부모이기 때문에 예외처리가 가능하다. 
