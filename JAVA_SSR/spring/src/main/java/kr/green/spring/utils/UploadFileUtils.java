...
public class UploadFileUtils {
	/* 기능    : 파일 주소와 파일명과 파일이 주어지면 해당 파일 주소에 파일을 업로드 하는 메소드
	 * 매개변수 : uploadPath - 업로드 할 파일 경로, originalName - 실제 파일명
	 *			fileData - 업로드할 실제 파일 데이터가 있는 배열
	 * 리턴타입 : 저장된 파일 경로와 파일명으로 된 문자열 
	 * 예시	  : uploadPath = "D:\\JAVA_SSR\\uploadfiles", originalName = "dog.jpg";
	...
	private static void makeDir(String uploadPath, String... paths) {
		//"D:\\JAVA_SSR\\uploadfiles\\2021\\07\\14
		//년월일로 된 폴더가 이미 있으면 메소드를 종료 
		if(new File(uploadPath + paths[paths.length-1]).exists())
			return;
		for(String path : paths) {
			//"D:\\JAVA_SSR\\uploadfiles\\2021\\7\\14
			File dirPath = new File(uploadPath + path);
			//파일(여기선 폴더)이 없으면 해당 폴더를 생성 
			if( !dirPath.exists())
				dirPath.mkdir();
		}
	}
	/* 기능 : 업로드 경로와 날짜 폴더 경로와 파일명이 주어지면 합쳐서 하나의 문자열로 반환하는 메소드
	 * 		 단, 경로에 \\ 대신 /로
	 * 
	 * */
