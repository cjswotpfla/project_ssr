/* 요구사항명세서를 통해 ERD 작성하느 과정이 DB설계 
SQL
- DDL
	- create : 테이블/DB를 생성
    - drop : 테이블/DB를 삭제
    - alter : 테이블/DB를 수정(속성, 제약조건 등을 수정)
- DML(CRUD)
  - insert : 튜블을 추가
  - select : 튜플을 검색
  - update : 튜플을 수정
...
- delete와 truncate를 이용하여 전체 튜플을 삭제하는 경우 비교
  - delete를 이용하여 데이터를 삭제하면 auto_increment를 가지는 속성이 있는 경우
    기존 값을 이어서 증가
  - truncate를 이용하여 데이터를 삭제하면 auto_increment를 가지는 속성이 있는경우 1부터 시작
  - truncate는 전체를 한번에 삭제하지만 delete는 한행씩 삭제해서 전체를 삭제하기 때문에 느림
  ...
  - 후보키(최소성) : 슈퍼키 중 필수적인 속성만으로 구성된 키
...
DB 복습
1. DB 설계(ERD) - 작은 규모의 시스템(과일가게에서 과일 판매시스템)부터 설계하는 연습을 하고,
	이후에 규모가 큰 시스템을 설계하는 연습을 해야함
